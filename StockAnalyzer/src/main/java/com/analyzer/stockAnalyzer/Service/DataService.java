...
public class DataService {
    ...
    public ArrayList<String> pandlList= new ArrayList<>();

    public ArrayList<Double> rrList = new ArrayList<>();



    public void startStockAnalysis(int tokenParam, String name, LocalDate dateParam) throws IOException {
        ...
        int currentVolume = 0;
        StringBuffer pAndL = new StringBuffer();
        StringBuffer rr = new StringBuffer();


        if (isGreen(enctoken, token, dateParam)) {
            int previousTenDaysSmaVolume = getPreviousTenDaysVolumeSma(enctoken, token, dateParam);
                            ...
                            if (responseCode == HttpURLConnection.HTTP_OK) {
                                ...
                                if (!candlesNode.isEmpty()) {
                                    for (JsonNode candle : candlesNode) {
                                        ...
                                        } else if (timeframe.equals("minute") && LocalDate.parse(date).equals(eventDate)) {
                                            //findTimestampExceedingSMA
                                            // Accumulate volume
                                            currentVolume += volume;

                                            System.out.println("current Time :" + time);
                                            if (time.equals(LocalTime.of(10, 50))) {
                                                System.out.println("wait");
                                            }
                                            // Check if cumulative volume exceeds SMA(10)
                                            int bigVolume = previousTenDaysSmaVolume * 10;
                                            if (currentVolume > bigVolume) {
                                                System.out.println("Buying timeStamp : " + date + " | " + time);
                                                System.out.println("Buying price");
                                                double roundPrice = Math.round(((high + low) / 2) * 100.0) / 100.0;
                                                System.out.println(roundPrice);
                                                historicalData.setSellingPrice(roundPrice);
                                                historicalData.setSellingPriceTimeStamp(time);
                                                break;
                                            }
                                        }
                                    }
            ...
            try {
                if (analyzedData.getSellingPrice() > 0){
                    saved = dataRepo.save(analyzedData);
                    rrList.add(saved.getPositionSellAtNineFifteenCloseRRRatio());
                    pandlList.add(saved.getPositionSellAtNineFifteenClosePAndL());
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private HistoricalData AnalyzeStocks(HistoricalData data) {
        ...
        double a = calculateRiskRewardRatio(data.getHigh(), data.getLow(), data.getSellingPrice(), data.getNdFifteenMin915AMClose());
        data.setPositionSellAtNineFifteenCloseRRRatio(Math.round(a * 100.0) / 100.0);
        if (a>0){
            data.setPositionSellAtNineFifteenClosePAndL("PROFIT");
        }else {
            data.setPositionSellAtNineFifteenClosePAndL("LOSS");
        }

        double b = calculateRiskRewardRatio(data.getHigh(), data.getLow(), data.getSellingPrice(), data.getNdFifteenMin915AMOpen());
        data.setPositionSellAtNineFifteenOpenRRRatio(Math.round(a * 100.0) / 100.0);
        if (b > 0){
            data.setPositionSellAtNineFifteenOpenPAndL("PROFIT");
        }else {
            data.setPositionSellAtNineFifteenOpenPAndL("LOSS");
        }
    ...
    public String readInputFile(String filePath, int limit, int skipNo) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (counter <= limit) {
                    ...
                    if (columns.length >= 2) { // Check if there are at least two columns
                        ...
                        try {
                            int intExact = Math.toIntExact(token);
                            startStockAnalysis(intExact, name, localDate);
                        }catch (Exception e){
                            System.out.println("some issue with stockName : "+name);
        ...
        writeAnalysedDataToFileforDouble(rrList,"rrList.txt");
        writeAnalysedDataToFileforString(pandlList,"pandlList.txt");
        return "done....";
    }

    public double calculateRiskRewardRatio( double high, double low, double entryPriceParam, double exitPriceParam) {
        // Entry price: 2 PM 15 min candle open
        double entryPrice = entryPriceParam;

        // Stop loss calculation
        double stopLoss = (0.25 * (high - low)) + high;

        // Exit price: next day 9:15 AM 15 min candle close
        double exitPrice = exitPriceParam;
    ...
    public int getPreviousTenDaysVolumeSma(String encTokenParam, int stockToken, LocalDate eventDate) throws IOException {

        String enctoken = encTokenParam;
        int token = stockToken;
        String timeframe = "day";
        int totalVolume = 0;
        int daysFetched = 0;
        LocalDate currentDate = eventDate.minusDays(1);
        int count = 0;
        while (daysFetched < 10 && count < 10) {

            LocalDate sdt = currentDate;
            LocalDate edt = currentDate;
            System.out.println("fetching volume for : " + currentDate);
            ...
            try {
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    if (!candlesNode.isEmpty()) {
                        for (JsonNode candle : candlesNode) {
                            System.out.println("Data found for date : " + currentDate);
                            int dayVolume = candle.get(5).asInt();
                            //get volume from json object
                            double volume = dayVolume;
                            // Assuming -1 indicates no response for the date
                            totalVolume += volume;
                            daysFetched++;

                        }
                    } else {
                        System.out.println("No Data found for date : " + currentDate);
                        count++;
                    }
                } else {
                    System.out.println("GET request failed. Response Code: " + responseCode);
        ...
        System.out.println("Sma of volume for last 10 days: " + totalVolume / 10);
        return totalVolume / 10;
    }

    public boolean isGreen(String encTokenParam, int stockToken, LocalDate eventDate) throws IOException {
        String enctoken = encTokenParam;
        int token = stockToken;
        String timeframe = "day";
        LocalDate currentDate = eventDate.minusDays(1);
    ...
    // Method to write SData objects to a file
    public static void writeAnalysedDataToFileforString(List<String> dataList, String filePath) {
        ...
        for (String data : dataList) {
            ...
            // Add a newline every 10 records
            if (count % 10 == 0) {
                stringBuffer.append("\n");
            }
        }

        // Ensure proper formatting: remove trailing comma or newline
        if (stringBuffer.length() > 0) {
            if (stringBuffer.charAt(stringBuffer.length() - 1) == ',') {
                stringBuffer.setLength(stringBuffer.length() - 1); // Remove trailing comma
            } else if (stringBuffer.charAt(stringBuffer.length() - 1) == '\n') {
                stringBuffer.setLength(stringBuffer.length() - 1); // Remove trailing newline
            }
        }
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath))) {
            ...
            System.out.println("Data written to file successfully: " + filePath);
        } catch (IOException e) {
            System.err.println("Error writing to file: " + e.getMessage());
        }
    }

    public static void writeAnalysedDataToFileforDouble(List<Double> dataList, String filePath) {
        StringBuffer stringBuffer = new StringBuffer();
        int count = 0;
